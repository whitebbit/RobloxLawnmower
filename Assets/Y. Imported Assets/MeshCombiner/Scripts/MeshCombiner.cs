...
public class MeshCombiner : MonoBehaviour
{
	...
	private List<MeshFilter> meshFiltersToSkip = new();
	...
	private MeshFilter[] GetMeshFiltersToCombine()
	{
		...
		meshFiltersToSkip = meshFiltersToSkip.Where((meshFilter) => meshFilter != meshFilters[0]).ToList();

		// Delete null values in meshFiltersToSkip array:
		meshFiltersToSkip = meshFiltersToSkip.Where((meshFilter) => meshFilter != null).ToList();

		for(int i = 0; i < meshFiltersToSkip.Count; i++)
		{
